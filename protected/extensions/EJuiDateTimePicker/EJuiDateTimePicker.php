...
class EJuiDateTimePicker extends CJuiDatePicker {
    ...
    protected function registerClientScript() {
        ...
        if ($this->language != 'en') {
            ...
            $cs->registerScript(
                    __CLASS__ . '#i18n-' . $this->language, "jQuery.datepicker.setDefaults(jQuery.datepicker.regional['{$this->language}']);", CClientScript::POS_READY
            );
        }
        ...
        if (isset($this->defaultOptions)) {
            $this->registerScriptFile($this->i18nScriptFile);
            $cs->registerScript(
                    __CLASS__ . '#mode-' . $this->mode, 'jQuery.' . $this->mode . 'picker.setDefaults(' . CJavaScript::encode($this->defaultOptions) . ');', CClientScript::POS_READY
            );
        }

        $selector = '#' . $this->htmlOptions['id'];
        $cs->registerScript(
                __CLASS__ . $selector, 'jQuery(' . CJavaScript::encode($selector) . ').' . $this->mode . 'picker(' . CJavaScript::encode($this->options) . ');', CClientScript::POS_READY
        );
    }
